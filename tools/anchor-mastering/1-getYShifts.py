...
# UC values will be shifted
for shift, chars in yShiftUC.items():
    
    strAccent = str(shift) + ": " + str(chars[0])
    
    if "stack" in strAccent:
        continue
    
    print ( str(shift) + ": " + str(chars[0]) )
    
    font [ str(chars[0]) ].selected = True
    font[chars[0]].prepareUndo("Shift y value")
    font[chars[0]].moveBy ( (0.0,shift ) )
    font[chars[0]].performUndo()
...
for shift, chars in yShiftlc.items():
    
    strAccent = ""
    
    print ( str(shift) + ": " + str(chars[0]) )
    
    if len( font[chars[0]].components ) > 0:
        strAccent = font[chars[0]].components[0].baseGlyph
        print ( font[chars[0]].components[0].baseGlyph )
    else:
        strAccent = font[chars[0]].name
    
    font[ strAccent ].selected = True
    font[ strAccent ].prepareUndo("Shift y value")
    font[ strAccent ].moveBy ( (0.0,shift ) )
    font[ strAccent ].performUndo()
